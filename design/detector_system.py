...
class Detector_System(object):  # detectors must be defined from upper left to upper right and then down each row
    ...
    def add_detector(self, **kwargs):
        new_detector = Detector(**kwargs)
        self.detectors.append(new_detector)
        if not len(self.detectors) == 1:
            self.sample_area = self.detectors[0].pix_size

        far_plane = (new_detector.norm * new_detector.c[2]) + (np.sign(new_detector.c[2]) * new_detector.thickness)
    ...
    def initialize_arrays(self):  # det_num, enter, total_intersection
        self.intersect_table = np.zeros(self.num_detectors, 3)
        self.intersect_table[:, 0] = np.arange(self.num_detectors)

    def _ray_projection(self, ray, em_dir):  # The goal of this function is to check for any intersections,
        ...
        for row, (mod_id, enter, total_intersection) in enumerate(valid_modules[valid_modules[:, 1].argsort()]):
            ...
            self.imager.projection[start_row:start_row+cur_det.npix[0], start_col:start_col+cur_det.npix[1]] = \
                cur_det._crystal_interaction_probabilities(ray, em_dir, self.imager.sample_step,
...
class Detector(object):
    ...
    def __init__(self, center=(0, 0, -200),  # mm Coordinates of the center face of
        ...
        self.det_id = det_id
        self.mod_id = np.array([det_id//4, det_id % 4])  # row, col
    ...
    def _check_intersection(self, ray_array):  # the ray should be generated elsewhere and only once from ray sampler
        intersection = (np.abs(np.dot(ray_array, self.axes[0])) < self.hist_ax0[-1]) & \
                       (np.abs(np.dot(ray_array, self.axes[1])) < self.hist_ax1[-1]) & \
    ...
    def _crystal_interaction_probabilities(self, ray_array, em_dir, step, enter, total_intersection, prefactor=1):
        ...
        prob_interact = np.exp(-self.mu * self.rho * step * np.arange(inside_rays.size)) * \
                        (1-np.exp(-self.mu * self.rho * step)) * \
                        area / (4 * np.pi * (step * np.arange(enter, enter + total_intersection))**2)

        return np.histogram2d(np.dot(inside_rays, self.axes[0]),
                              np.dot(inside_rays, self.axes[1]),
                              bins=(self.hist_ax0, self.hist_ax1),
                              weights=prefactor * prob_interact)[0]  # normalize by subpixels

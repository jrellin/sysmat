...
class Imager(object):
    ...
    #def generate_ray(self, src_pt, det_pt):  # generates the ray once  # TODO: Original
    ...
    def generate_ray(self, src_pt, det_pt):  # generates the ray once  # TODO: Rewrite to be much shorter
        em_dir = norm(det_pt - src_pt)  # TODO: do the same for collimator and append two arange calls

        dir_col = np.sign(self.collimator.colp[2] - self.sources.sc[2])  # in z-hat, pointing to detector
        min_sc_coll = (0.99 * (self.collimator.colp[2] - (dir_col * self.collimator.col_half_thickness)) - src_pt[2]) \
                      / em_dir[2]
        max_sc_coll = (1.01 * (self.collimator.colp[2] + (dir_col * self.collimator.col_half_thickness)) - src_pt[2]) \
                      / em_dir[2]

        min_sc_det = (0.99 * self.detector_system.closest_plane - src_pt[2]) / em_dir[2]
        max_sc_det = (1.01 * self.detector_system.farthest_plane - src_pt[2]) / em_dir[2]  # This is a max length in mm
        return src_pt + em_dir * np.r_[np.arange(min_sc_coll, max_sc_coll + self.sample_step, self.sample_step),
                                       np.arange(min_sc_det, max_sc_det + self.sample_step,
                                                 self.sample_step)][:, np.newaxis], em_dir
...
def main_table():
    ...
    print("Closest Plane: ", system.detector_system.closest_plane)
